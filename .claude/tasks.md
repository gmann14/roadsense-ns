...
- [ ] Android pothole photo CameraX UI — backend half landed (DTOs, `BackendClient.beginPotholePhotoUpload`, `PotholePhotoCoordinator` two-step drainer wired into `UploadDrainWorker`, JSON parity with iOS pinned by `PotholePhotoRequestJsonTest`). Remaining: in-Compose camera capture flow that writes a JPEG to app-internal storage, computes the SHA-256, and inserts a `PotholePhotoEntity` in `PhotoUploadState.PENDING_METADATA`. Capture is gated on the `CAMERA` permission, on `BuildConfig.ENABLE_POTHOLE_PHOTOS`, and on a successful prior manual pothole report (so the photo binds to a known `report_id`). UI work benefits from real-device validation; sequence behind the first Pixel/Samsung field cycle.

## Done

- [x] Shipped the two SQL-only pre-launch trust gates from `docs/implementation/17-device-attestation-and-trust.md`: B166 exact unique-contributor dedupe (`segment_contributor_marks`, closes review P1-3) and B165 pothole corroboration gating (`candidate` status + `pothole_reporter_marks`, ≥2 distinct devices to publish, closes review P1-2; TestFlight-era actives grandfathered). Commits `fca1dc2`, `cc9bcbe`. Owner follow-up: deploy migrations + redeploy Railway service; pgTAP suites 017/018 not yet executed locally (Docker stack was down) — run `supabase test db` before deploy.
