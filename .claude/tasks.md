...
- [ ] Android native Mapbox vector-tile + pothole overlay rendering. The Compose `MapHost` already swaps in the real Mapbox `MapView` whenever `MAPBOX_DOWNLOADS_TOKEN` is exported (env var or `~/.gradle/gradle.properties`); the remaining work is provisioning the token, wiring the same vector-tile source iOS uses, and porting the road-quality + pothole layer styles. Until the token is provisioned the map host falls back to a WebView pointing at the public web map.
