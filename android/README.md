...
- Pothole photo CameraX UI. Backend half landed: `PotholePhotoUploadRequest`/`Response` DTOs (`core-api`), `BackendClient.beginPotholePhotoUpload` + signed-URL `PUT`, and `PotholePhotoCoordinator` that drains `pothole_photos` rows on the unified `UploadDrainWorker` heartbeat (POST metadata → PUT bytes → `pending_moderation`). Remaining: the in-app camera capture flow that creates the local `PotholePhotoEntity` rows. The action flow (manual report + upload) ships in beta; the photo capture UI sequences behind first Pixel/Samsung field cycle.
