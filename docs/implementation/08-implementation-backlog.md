...
- **Current repo note:** This slice is now materially implemented: `MapScreen` replaced the debug shell, `RoadQualityMapView` can render live backend vector tiles through Mapbox, potholes render on-map, pending local drives render as a dashed teal overlay, segment taps highlight via feature-state, the existing `SegmentDetailSheet` is presented from real `GET /segments/{id}` fetches, and simulator UI smokes cover shell/settings/privacy-editor navigation through a deterministic non-Mapbox testing surface. Remaining work is deeper drawer-selection UI coverage and real-device field validation.
- **Release hardening note (2026-05-29):** TestFlight builds `1.0 (25)` and `1.0 (26)` produced launch `SIGTRAP` reports while Mapbox SwiftUI was updating its `UIViewControllerRepresentable`. Build `1.0 (26)` proved the issue was not limited to follow-puck startup, so TestFlight/App Store builds must keep `ENABLE_LIVE_MAPBOX_MAP=NO` until a real-device red/green pass isolates the Mapbox SwiftUI crash. Build `1.0 (28)` used a native MapKit fallback for launch containment. Build `1.0 (29)` restores the public road-quality vector tile overlay through a UIKit-backed Mapbox view behind `ENABLE_ROAD_QUALITY_VECTOR_OVERLAY=YES`, while local placeholder-token builds keep that flag off.
...
- **Current repo note:** `BackgroundCollectionPolicy` (now wired into `AppModel`/`LocationService` to gate `allowsBackgroundLocationUpdates` on the Always tier), `BackgroundTaskRegistrar`, aligned background task IDs (`nightly-cleanup`, `upload-drain`), significant-location-change passive monitoring, moving-GPS collection bootstrap, fresh-checkpoint service resume, and longer local sample retention for mark/photo actions are now implemented. Power tiers are governed by `LocationActivationPolicy`: continuous high-accuracy GPS runs only while a drive is being recorded or while the app is in the foreground; backgrounded passive standby uses significant-location-change monitoring only. Settings also exposes drive diagnostics for the last GPS sample, driving signal, collection start/stop, and bump candidate. Remaining work is signed real-device validation for lock-screen, background, and system-termination scenarios, plus the deferred battery-saver sampling mode (reduced Hz/GPS cadence under Low Power Mode and thermal pressure).
...
- **Status:** internal collection beta implementation landed in `android/`: core-sensor fixture parity, core-api upload contracts, Room queue, foreground collection service, launcher beta control shell, WorkManager upload drain, fixture-parity script, and Android CI workflow. Full production polish remains open: real-device Pixel/Samsung validation, battery/foreground-service evidence, native Mapbox map shell, manual pothole/photo flow, feedback queue, signed AAB, and Play Console submission. See [12-android-implementation.md](12-android-implementation.md) and [15-google-play-readiness.md](15-google-play-readiness.md).
- **RED**
