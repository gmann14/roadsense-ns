...
- **Release hardening note (2026-05-29):** TestFlight builds `1.0 (25)` and `1.0 (26)` produced launch `SIGTRAP` reports while Mapbox SwiftUI was updating its `UIViewControllerRepresentable`. Build `1.0 (26)` proved the issue was not limited to follow-puck startup, so TestFlight/App Store builds must keep `ENABLE_LIVE_MAPBOX_MAP=NO` until a real-device red/green pass isolates the Mapbox crash. Re-enable the live in-app map only after package/app tests and device validation pass.
