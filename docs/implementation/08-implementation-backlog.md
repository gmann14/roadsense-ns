...
- **Release hardening note (2026-05-29):** TestFlight builds `1.0 (25)` and `1.0 (26)` produced launch `SIGTRAP` reports while Mapbox SwiftUI was updating its `UIViewControllerRepresentable`. Build `1.0 (26)` proved the issue was not limited to follow-puck startup, so TestFlight/App Store builds must keep `ENABLE_LIVE_MAPBOX_MAP=NO` until a real-device red/green pass isolates the Mapbox crash. Build `1.0 (28)` should use a native MapKit fallback for the launch surface so testers see a map without instantiating Mapbox on app open. Re-enable the live in-app Mapbox overlay only after package/app tests and device validation pass.
