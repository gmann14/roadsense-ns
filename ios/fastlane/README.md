...
- `ENABLE_LIVE_MAPBOX_MAP=YES` restores the legacy SwiftUI Mapbox map. Leave it `NO` for TestFlight/App Store builds until the build 26 launch trap is closed on real devices.
- `ENABLE_ROAD_QUALITY_VECTOR_OVERLAY=YES` enables the UIKit-backed Mapbox road-quality vector overlay. The GitHub TestFlight workflow sets it to `YES`; local debug keeps it `NO` unless a real Mapbox token is configured.
