...
AS $$
BEGIN
    -- B014 implements real pothole folding. Keep the callable seam in place so
    -- ingest_reading_batch remains runnable and atomic before pothole logic lands.
    PERFORM p_batch_id;
    RETURN;
END;
$$;
...
REVOKE EXECUTE ON FUNCTION update_segment_aggregates_from_batch(UUID) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION update_segment_aggregates_from_batch(UUID) FROM anon;
REVOKE EXECUTE ON FUNCTION update_segment_aggregates_from_batch(UUID) FROM authenticated;
GRANT EXECUTE ON FUNCTION update_segment_aggregates_from_batch(UUID) TO service_role;

REVOKE EXECUTE ON FUNCTION fold_pothole_candidates(UUID) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION fold_pothole_candidates(UUID) FROM anon;
REVOKE EXECUTE ON FUNCTION fold_pothole_candidates(UUID) FROM authenticated;
GRANT EXECUTE ON FUNCTION fold_pothole_candidates(UUID) TO service_role;
