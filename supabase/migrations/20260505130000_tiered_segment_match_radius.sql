-- Vary segment-match radius by road class.
--
-- Highway driving regularly produces readings 25-45m off OSM centerlines
-- because (a) divided highways share one centerline, (b) on/off ramps
-- and shoulder lanes drift, (c) GPS accuracy spikes near overpasses
-- and in cuts. The previous flat 20m cutoff was rejecting readings on
-- Hwy 102/103 stretches the user actually drove. Telemetry as of
-- 2026-05-05 showed ~1,863 readings dropped with `no_segment_match`
-- vs 2,135 accepted — close to a 1:1 reject ratio.
--
-- City streets stay tight (20m) so a reading near an intersection still
-- attaches to the right block.
--
-- Radius tiers:
--   motorway / trunk / primary + _link   → 50m initial bbox, 45m final
--   secondary / tertiary + _link         → 30m initial, 25m final
--   everything else (residential, etc.)  → 25m initial, 20m final
--
-- Reference: docs/implementation/08-implementation-backlog.md (B072 below).
